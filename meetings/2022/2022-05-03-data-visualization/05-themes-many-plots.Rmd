...
We'll also make use of a new package **cowplot**
...
```{r in-built-themes-5, fig.width = 5, fig.height = 4, dpi = 200, fig.show = "hold", dev = 'svg', out.width = "500cm", out.height = "400cm"}
labelled
theme_set(theme_bw())
labelled
```
...
* the **cowplot** package is the one I use most
* **patchwork** looks easier but not on CRAN yet

When you load **cowplot** it also loads an **opinionated** theme &mdash; reset
...
**cowplot** works by taking multiple pre-prepared plots and arranging them on the device
...
plot_grid(p1, p2, p3, p4, ncol = 2, labels = "auto", align = 'hv', axis = 'lrtb')
```
